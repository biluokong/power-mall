...
export default {
  ...
  methods: {
      ...
      if (this.dataForm.addrId) {
        this.$http({
          url: this.$http.adornUrl(
            `/store-service/shop/pickAddr/info/${this.dataForm.addrId}`
          ),
          method: 'get',
    ...
    listAreaByParentId (pid) {
      if (!pid) pid = 0
      return this.$http({
        url: this.$http.adornUrl(`/store-service/admin/area/listByPid`),
        method: 'get',
    ...
    dataFormSubmit () {
      ...
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(`/store-service/shop/pickAddr`),
