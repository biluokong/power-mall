                ...
                <div class="item">
                  <div class="operate">
                    ...
                    <el-button v-if="isAuth('order:order:update')"
                               type="text"
                               size="small"
                               @click="addOrUpdateHandle(order.orderNumber)">详情</el-button>
                  </div>
                </div>
              </el-col>
            </el-row>
...
export default {
  ...
  methods: {
    ...
    getDataList (page, params) {
      ...
      this.$http({
        url: this.$http.adornUrl('/order-service/order/order/page'),
        method: 'get',
    ...
    getSoldExcel () {
      this.$http({
        url: this.$http.adornUrl('/order-service/order/order/soldExcel'),
        method: 'get',
        params: this.$http.adornParams({
          'startTime': this.dateRange === null ? null : this.dateRange[0], // 开始时间
          'endTime': this.dateRange === null ? null : this.dateRange[1] // 结束时间
